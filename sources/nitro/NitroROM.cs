...
namespace SM64DSe.sources.nitro
{
    ...
    public partial class NitroROM
    {
        ...
        /**
         * This function take as argument a parameter value (used by sign for example)
         * and will return the mapping 
         */
        public ushort GetMessageIdByParameterValue(ushort id)
        {
            ...
            ushort[] shorts = new ushort[98];
            for (int i = 0; i < 98; i++)
            {
                shorts[i] = (ushort)(block[2*i] | (block[2*i+1] << 8));
            }
        ...
        {
            // Assume you already have a BinaryWriter object named 'writer'
            m_BinReader.BaseStream.Position = 0;

            // Create a FileStream to write to the binary file
            using (FileStream fileStream = new FileStream("example.bin", FileMode.Create))
            {
                ...
                {
                    // Read data from the Stream and write it to the binary file
                    byte[] buffer = new byte[4096];
                    int bytesRead;
                    while ((bytesRead = m_BinReader.Read(buffer, 0, buffer.Length)) > 0)
                    {
