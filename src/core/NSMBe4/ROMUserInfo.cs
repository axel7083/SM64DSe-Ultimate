    ...
    public class ROMUserInfo
    {
        ...
        public ROMUserInfo(string ROMPath)
        {
            ...
            try
            {
                ...
                DSFileSystem.NSMBe4File nsmBe4File = NSMBe4ROM.FS.getFileByName("00DUMMY");
                string[] lines = System.Text.Encoding.ASCII.GetString(nsmBe4File.getContents()).Split('\n');
        ...
        public List<string> getFullList(string name)
        {
            ...
            if (name == "Music")
                for (int l = 0; l < NSMBe4ROM.MusicNumbers.Length; l++)
                    newList[l] = string.Format("{0:X2}: {1}", NSMBe4ROM.MusicNumbers[l], newList[l]);
        ...
        public void SaveFile()
        {
            try
            {
                ...
                DSFileSystem.NSMBe4File nsmBe4File = NSMBe4ROM.FS.getFileByName("00DUMMY");
                nsmBe4File.beginEdit(this);
                nsmBe4File.replace(System.Text.Encoding.ASCII.GetBytes(text), this);
                nsmBe4File.endEdit(this);
            }
            catch (Exception ex)
            {
