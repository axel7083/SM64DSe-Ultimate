...
namespace SM64DSe.core.NSMBe4.Patcher
{
    public class Arm9BinaryHandler
    {
        ...
        {
            f = NSMBe4ROM.Arm9BinNsmBe4File;
            this.fs = NSMBe4ROM.FS;
        }
        ...
        public void newSection(int ramAddr, int ramLen, int fileOffs, int bssSize)
        {
            ...
            byte[] content = f.getContents();
            
            
            byte[] data = new byte[ramLen];
            Array.Copy(content, fileOffs, data, 0, ramLen);
        ...
        public void loadSections()
        {
            ...
            int copyTableBegin = (int)(f.getUintAt(getCodeSettingsOffs() + 0x00) - NSMBe4ROM.arm9RAMAddress);
            int copyTableEnd = (int)(f.getUintAt(getCodeSettingsOffs() + 0x04) - NSMBe4ROM.arm9RAMAddress);
            int dataBegin = (int)(f.getUintAt(getCodeSettingsOffs() + 0x08) - NSMBe4ROM.arm9RAMAddress);

            newSection((int)NSMBe4ROM.arm9RAMAddress, dataBegin, 0x0, 0);
        ...
        public void saveSections()
        {
            ...
            f.beginEdit(this);
            NSMBe4ByteArrayOutputStream o = new NSMBe4ByteArrayOutputStream();
            ...
            uint sectionTableAddr = NSMBe4ROM.arm9RAMAddress + 0xE00;
            NSMBe4ByteArrayOutputStream o2 = new NSMBe4ByteArrayOutputStream();
            foreach (Arm9BinSection s in sections)
            {
                if (!s.real) continue;
                if (s.len == 0) continue;
            ...
            Array.Copy(sectionTable, 0, data, sectionTableAddr - NSMBe4ROM.arm9RAMAddress, sectionTable.Length);
            f.replace(data, this);
            f.endEdit(this);
            ...
            f.setUintAt(getCodeSettingsOffs() + 0x08, (uint)(sections[0].len + NSMBe4ROM.arm9RAMAddress));
        ...
        public int decompressionRamAddr
        {
            get
            {
                int value = (int)f.getUintAt(getCodeSettingsOffs() + 0x14);
                return value;
            }
        ...
        public void decompress()
        {
            ...
            int decompressionOffs = decompressionRamAddr - (int)NSMBe4ROM.arm9RAMAddress;
            ...
            byte[] decompData = NSMBe4ROM.DecompressOverlay(compData);
        ...
        public void writeToRamAddr(int ramAddr, uint val, int ov)
        {
			if(ov != -1)
			{
		        foreach(NSMBe4Overlay of in NSMBe4ROM.arm9ovs)
		        	if(of.ovId == ov)
		        	{
        ...
        {
            System.IO.DirectoryInfo dir = new System.IO.DirectoryInfo(NSMBe4ROM.romfile.Directory.FullName+"/bak");
            //Console.Out.WriteLine("Backing up " + file + " "+dir.FullName);
            if (!dir.Exists)
                dir.Create();
            
            dir = NSMBe4ROM.romfile.Directory;
            ...
            NSMBe4File f = NSMBe4ROM.Arm9BinNsmBe4File;
            if (file != -1)
            {
                f = NSMBe4ROM.arm9ovs[file].f;
                NSMBe4ROM.arm9ovs[file].decompress();
            }
        ...
        public void restoreFromBackup()
        {
            ...
            foreach (System.IO.FileInfo f in dir.GetFiles())
            {
                ...
                NSMBe4File ff = null;
				NSMBe4Overlay oo = null;
				
                n = n.Substring(0, n.Length - 4);
                if (n == "main")
                    ff = NSMBe4ROM.Arm9BinNsmBe4File;
                else
                {
                    int num = 0;
                    if (Int32.TryParse(n, out num))
                    {
                    	oo = NSMBe4ROM.arm9ovs[num];
                        ff = oo.f;
                    }
                }
