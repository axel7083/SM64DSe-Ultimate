...
namespace SM64DSe
{
    public class BCA
    {
        ...
        {
            var matrices = new Matrix4[chunks.Length];
        ...
        {
            var localSRTValues = new SRTContainer[numChunks];
        ...
        {
            var localSRTValues = new SRTContainer[m_NumFrames];
        ...
        public class AnimationData
        {
            ...
            public AnimationData(BCA bca, uint offset, int boneID)
            {
                ...
                m_TranslationXValues =
                    ReadValuesForDescriptor20_12Float(m_BCA.m_TranslationValuesOffset, m_TranslationX);
                m_TranslationYValues =
                    ReadValuesForDescriptor20_12Float(m_BCA.m_TranslationValuesOffset, m_TranslationY);
                m_TranslationZValues =
                    ReadValuesForDescriptor20_12Float(m_BCA.m_TranslationValuesOffset, m_TranslationZ);
            }
            ...
            {
                var srt = GetScaleRotationTranslation(frame);

                var ret = srt.m_Matrix;
                    ...
                    if (descriptor.m_Interpolate == 1)
                    {
                        ...
                        if ((frameNum & 1) != 0)
                        {
                            ...
                            else // else interpolate between current and next values
                            {
                                ...
                                */
                                var val1 = values[frameNum >> 1];
                                var val2 = values[(frameNum >> 1) + 1];
                                if (isRotation)
                                {
                                    if (val1 < 0f && val2 > 0f)
                                    {
            ...
            private float[] ReadValuesForDescriptor20_12Float(uint offset, AnimationDescriptor descriptor)
            {
                return descriptor.m_ConstantValue
                    ...
                    : Read20_12Floats((uint)(offset + descriptor.m_StartOffset * 4),
                        (int)(m_BCA.m_NumFrames / (descriptor.m_Interpolate == 1 ? 2u : 1u) +
                              (uint)descriptor.m_Interpolate));
            }
        ...
        public class SRTContainer
        {
            ...
            {
                var rotDeg = new Vector3(m_Rotation.X * Helper.Rad2Deg, m_Rotation.Y * Helper.Rad2Deg,
                    m_Rotation.Z * Helper.Rad2Deg);
